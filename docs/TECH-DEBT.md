...
- [x] **4.1** Schemas zod criados em `src/schemas/`: `member.ts` (com `isValidCPF` + `memberFormSchema` + `cpfSchema`), `seminario.ts` (`seminarioFormSchema` + `matriculaFormSchema` com refine cruzado de datas), `carteirinha.ts` (`carteirinhaGerarSchema` + helpers puros `calcularValidadeCarteirinha` e `gerarNumeroSequencial`). `SeminarioModal` migrado pra `react-hook-form + zodResolver` como referência de padrão — exibe `errors.<campo>` inline com `aria-invalid` e `aria-describedby`. Demais modais migram quando tocarmos neles (custo alto, não urgente). `TabPerfil` passou a reutilizar `isValidCPF` do schema.
- [x] **4.2** Vitest + @testing-library/react + jsdom instalados. Config em `vite.config.ts`. Scripts `npm test` e `npm run test:watch`. **34 testes unitários criados** em 3 arquivos: `member.test.ts` (CPF + schema), `carteirinha.test.ts` (validade + numeração + schema), `AdvancedSearch.test.ts` (11 testes dos filtros — apelido, faixa etária, períodos, CPF, cidade, celular). Todos passando.
- [x] **4.3** `components/ui/Skeleton.tsx` criado com `<Skeleton />` genérico (aceita `rounded` e `className`) e `<TableRowsSkeleton />`. `PageLoader.tsx` foi refeito pra exibir skeletons com estrutura típica de página (header + KPIs + tabela) em vez de só um spinner.
- [x] **4.4** A11y crítica: `aria-label` em botões-ícone da Topbar (Notificações + `aria-haspopup`/`aria-expanded`, E-mail, Sair). `role="dialog"` + `aria-modal="true"` + `aria-labelledby` em `MemberModal` e `MemberViewModal`. Botão X de fechar ganhou `aria-label`. `<Skeleton>` tem `role="status"` + `aria-busy`. Botões com `title=` nas linhas de ação já são lidos por screen readers; cobertura mais ampla fica pra quando formos tocar nesses arquivos.
- [x] **4.5** Removido `(form as any).notes` do `VisitanteModal`. Tipo `Member` agora tem `notes?: string`.
...
- [x] **2026-04-24** — **Sprint 4 concluído.** Schemas zod em `src/schemas/` (member, seminario, carteirinha) com validação de CPF centralizada + helpers puros. `SeminarioModal` migrado pra `react-hook-form + zodResolver` como referência. Vitest + 34 testes unitários passando (CPF, schemas, filtros da AdvancedSearch, validade de carteirinha, numeração). `Skeleton.tsx` criado + `PageLoader` redesenhado. A11y: `role=dialog` + `aria-labelledby` + `aria-label` em botões-ícone críticos. `as any` do VisitanteModal removido (tipo `Member.notes` adicionado). Bundle inicial: 765 KB gzip 229 KB (+29 KB pro react-hook-form na SeminariosPage, ainda saudável).
