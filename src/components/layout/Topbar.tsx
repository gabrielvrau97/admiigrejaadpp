...
import { Bell, Mail, LogOut, Globe, ChevronDown, Church } from 'lucide-react'
...
export default function Topbar() {
  ...
  return (
    <header className="fixed top-0 left-56 right-0 h-12 bg-white/95 backdrop-blur-sm border-b border-gray-200/80 flex items-center px-5 gap-3 z-20"
      ...
      <div className="relative">
        <button
          ...
          className="flex items-center gap-1.5 text-sm font-medium text-gray-600 hover:text-blue-600 transition-all duration-150 hover:bg-blue-50 px-2 py-1 rounded-md -mx-2"
        >
          <Church size={14} className="text-blue-500" />
          <span>{selectedChurch ? selectedChurch.name : 'Sede e filiais'}</span>
          <ChevronDown size={12} className={`transition-transform duration-200 ${showChurchDD ? 'rotate-180' : ''}`} />
        </button>
        {showChurchDD && (
          <>
            <div className="fixed inset-0 z-40" onClick={() => setShowChurchDD(false)} />
            <div className="absolute top-full left-0 mt-1.5 w-52 bg-white border border-gray-200/80 rounded-lg z-50 py-1 overflow-hidden"
      ...
      {/* Session Timer */}
      <div className={`flex items-center gap-1.5 text-xs font-mono px-2.5 py-1 rounded-md font-medium transition-all ${
        isWarning
      ...
      <button
        ...
        className="flex items-center gap-1 text-xs text-gray-500 hover:text-blue-600 transition-all duration-150 hover:bg-blue-50 px-2 py-1 rounded-md font-medium"
      >
        <Globe size={13} />
        <span>{lang}</span>
      </button>
      ...
      <div className="relative">
        ...
        {showNotif && (
          <>
            ...
            <div className="absolute top-full right-0 mt-1.5 w-72 bg-white border border-gray-200/80 rounded-xl z-50 overflow-hidden"
      ...
      {/* Mail */}
      <button className="p-1.5 text-gray-400 hover:text-blue-600 transition-all duration-150 hover:bg-blue-50 rounded-md">
        <Mail size={17} />
      </button>
