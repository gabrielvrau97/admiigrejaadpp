...
import { X, FileText, PenLine, UserCheck } from 'lucide-react'
...
export default function RelatorioAssinaturaModal({ onConfirm, onClose }: Props) {
  ...
  // Pré-carrega pastor e tesoureiros
  useEffect(() => {
    async function load() {
      try {
        const [churches, tesoureiros] = await Promise.all([
          listChurches(),
                      ...
                      <div className="px-3 pb-2.5 grid grid-cols-2 gap-2">
                        <div>
                          ...
                          <input
                            type="text"
                            value={s.nome}
                            onChange={e => setNome(s.key, e.target.value)}
                            placeholder="Nome..."
